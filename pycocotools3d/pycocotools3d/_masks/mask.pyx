...
cdef class Masks:
    ...
    def __cinit__(self, h, w, n):
        self.mask = <byte *> malloc(h * w * n * sizeof(byte))
        self.h = h
        self.w = w
        self.n = n
    # def __dealloc__(self):
    ...
    def __array__(self):
        ...
        shape[0] = <np.npy_intp> self.h * self.w * self.n
        ...
        ndarray = np.PyArray_SimpleNewFromData(1, shape, np.NPY_UINT8, self.mask) \
            .reshape((self.h, self.w, self.n), order='F')
...
def _toString(RLEs Rs):
    ...
    objs = []
    for i in range(n):
        c_string = rleToString(<RLE *> &Rs.R[i])
        py_string = c_string
        objs.append({
            'size': [Rs.R[i].h, Rs.R[i].w],
            'counts': py_string
        })
        free(c_string)
    return objs
...
def _frString(rleObjs):
    ...
    for i, obj in enumerate(rleObjs):
        ...
        c_string = py_string
        rleFrString(<RLE *> &Rs.R[i], <char *> c_string, obj['size'][0], obj['size'][1])
    return Rs
...
def encode(np.ndarray[np.uint8_t, ndim=3, mode='fortran'] mask):
    ...
    cdef RLEs Rs = RLEs(n)
    rleEncode(Rs.R, <byte *> mask.data, h, w, n)
    objs = _toString(Rs)
    return objs
...
def decode(rleObjs):
    ...
    h, w, n = Rs.R[0].h, Rs.R[0].w, Rs.n
    masks = Masks(h, w, n)
    if rleDecode(<RLE *> Rs.R, masks.mask, n) != 1:
...
def merge(rleObjs, intersect=0):
    cdef RLEs Rs = _frString(rleObjs)
    cdef RLEs R = RLEs(1)
    rleMerge(<RLE *> Rs.R, <RLE *> R.R, <siz> Rs.n, intersect)
    obj = _toString(R)[0]
    return obj
...
def iou(dt, gt, pyiscrowd):
    def _preproc(objs):
        ...
        if type(objs) == np.ndarray:
            ...
            is_box = np.all(np.array([(len(obj) == 4) and
            ...
            raise Exception('unrecognized type.  The following type: RLEs (rle), np.ndarray (box), '
                            'and list (box) are supported.')
        return objs
    def _rleIou(RLEs dt, RLEs gt, np.ndarray[np.uint8_t, ndim=1] iscrowd, siz m, siz n,
                np.ndarray[np.double_t, ndim=1] _iou):
        rleIou(<RLE *> dt.R, <RLE *> gt.R, m, n, <byte *> iscrowd.data, <double *> _iou.data)
...
def toBbox(rleObjs):
    ...
    rleToBbox(<const RLE *> Rs.R, _bb, n)
    cdef np.npy_intp shape[1]
...
def frBbox(np.ndarray[np.double_t, ndim=2] bb, siz h, siz w):
    ...
    Rs = RLEs(n)
    rleFrBbox(<RLE *> Rs.R, <const BB> bb.data, h, w, n)
    objs = _toString(Rs)
    return objs
...
def frPoly(poly, siz h, siz w):
    ...
    for i, p in enumerate(poly):
        ...
        rleFrPoly(<RLE *> &Rs.R[i], <const double *> np_poly.data, int(len(p) / 2), h, w)
    objs = _toString(Rs)
    return objs
...
def frPyObjects(pyobj, h, w):
    ...
    # (not syntactically but semantically)
            ...
            len(pyobj[0]) == 4:
        objs = frBbox(np.array(pyobj, np.float64), h, w)
    elif isinstance(pyobj, list) and \
            (isinstance(pyobj[0], list) or isinstance(pyobj[0], np.ndarray)) and \
            len(pyobj[0]) > 4:
    ...
    # encode rle from single python object
        ...
        objs = frBbox(np.array([pyobj], np.float64), h, w)[0]
    elif type(pyobj) == list and len(pyobj) > 4:

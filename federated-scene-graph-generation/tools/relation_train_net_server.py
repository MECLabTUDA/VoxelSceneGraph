...
import click
from fed_scene_graph_prediction.theoden_extensions.commands.implementation.LoadStateDictExceptStatisticsCommand import \
    LoadStateDictExceptStatisticsCommand
...
from fed_scene_graph_prediction.theoden_extensions.commands import InitModelRelationDetectionCommand, \
...
from theoden.operations import ClosedDistribution, RequireNumberOfClientsCondition, ExitRunCommand, SequentialCommand, \
    SendModelToServerCommand, AggregationAction, FedAvgAggregator
        ...
        instructions=[
            ...
            ClosedDistribution(
                commands=[SequentialCommand([
                    ...
                    InitModelRelationDetectionCommand(),
                    # FIXME set model/optimizer keys somewhere + only_grad
                    # Note: since the statistics in all clients are automatically initialized,
                    #       we just have to avoid overwriting them before aggregating again
                    LoadStateDictExceptStatisticsCommand(resource_key="model"),
                    SendModelToServerCommand("model", only_grad=False),
                ])]
            ),
            AggregationAction(aggregator=FedAvgAggregator()),
            ClosedDistribution(LoadTrainValDatasetCommand()),

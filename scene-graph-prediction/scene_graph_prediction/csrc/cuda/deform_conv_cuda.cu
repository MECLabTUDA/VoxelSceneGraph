                 ...
                 int deformable_group) 
{
  ...
  long inputHeight = input.size(dim_h);
  long inputWidth = input.size(dim_w);
  ...
  if (gradOutput != NULL) {
    AT_CHECK(gradOutput->size(dim_f) == nOutputPlane,
             "invalid number of gradOutput planes, expected: %d, but got: %d",
             nOutputPlane, gradOutput->size(dim_f));

    AT_CHECK((gradOutput->size(dim_h) == outputHeight &&
              gradOutput->size(dim_w) == outputWidth),
             "invalid size of gradOutput, expected height: %d width: %d , but "
             "got height: %d width: %d",
             outputHeight, outputWidth, gradOutput->size(dim_h),
             gradOutput->size(dim_w));
  }
}
...
{
  // c_todo: resize columns to include im2col: done
  // c_todo: add im2col_step as input
  // c_todo: add new output buffer and transpose it to output (or directly transpose output)
  // c_todo: possibly change data indexing because of parallel_imgs
  ...
  // c_todo: assert batch size dividable by im2col_step
  ...
  at::Tensor output_buffer =
      at::zeros({batchSize / im2col_step, nOutputPlane, im2col_step * outputHeight, outputWidth}, output.options());
...
{
  // c_todo: transpose and reshape outGrad
  // c_todo: reshape columns
  // c_todo: add im2col_step as input

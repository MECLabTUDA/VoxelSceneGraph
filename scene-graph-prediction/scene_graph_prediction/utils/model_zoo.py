...
def cache_url(url: str, model_dir: str | None = None, progress: bool = True) -> str:
    """
    Loads the Torch serialized object at the given URL.
    If the object is already present in `model_dir`, it's deserialized and returned.
    The filename part of the URL should follow the naming convention ``filename-<sha256>.ext`` where ``<sha256>``
    ...
    The default directory can be overridden with the ``$TORCH_MODEL_ZOO`` environment variable.
    :param url: URL of the object to download
    :param model_dir: directory in which to save the object
    :param progress: whether to display a progress bar to stderr

...
def reduce_dict(input_dict: dict, average: bool = True) -> dict:
    """
    Reduce across GPUs.
    Reduce the values in the dictionary from all processes so that process with rank 0 has the averaged results.
    Returns a dict with the same fields as input_dict, after reduction.
    :param input_dict:
    :param average: if True, the average is computed otherwise a sum is computed
    """
    world_size = get_world_size()
    if world_size < 2:
        return input_dict

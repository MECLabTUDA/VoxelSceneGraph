...
class ROIMaskHead(AbstractROIMaskHead):
    def __init__(self, cfg: CfgNode, in_channels: int, anchor_strides: AnchorStrides):
        ...
        assert self.feature_extractor.is_mask_head_compatible, "The feature extractor should be compatible."
        self.predictor = build_roi_mask_predictor(cfg, self.feature_extractor.output_size())
        self.post_processor = build_roi_mask_post_processor(cfg)
        self.loss_evaluator = build_roi_mask_loss_evaluator(cfg, self.predictor.output_size())

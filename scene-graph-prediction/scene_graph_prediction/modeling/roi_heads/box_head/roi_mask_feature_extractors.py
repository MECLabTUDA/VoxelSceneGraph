...
from scene_graph_prediction.modeling.abstractions.box_head import ROIBoxMaskFeatureExtractor
from scene_graph_prediction.modeling.registries import *


@ROI_BOX_MASK_FEATURE_EXTRACTORS.register("Factor444RelationMaskFeatureExtractor")
class Factor444RelationMaskFeatureExtractor(ROIBoxMaskFeatureExtractor):
    ...
    def __init__(self, cfg: CfgNode, in_channels: int, out_channels: int):
        super().__init__(cfg, in_channels, out_channels)
        ...
        if self.n_dim == 2:
            self.rect_conv = nn.Sequential(
                nn.Conv2d(in_channels, out_channels // 2, kernel_size=7, stride=2, padding=3, bias=True),
            ...
            self.rect_conv = nn.Sequential(
                nn.Conv3d(in_channels, out_channels // 2, kernel_size=7, stride=2, padding=3, bias=True),
                nn.ReLU(inplace=True),
...
@ROI_BOX_MASK_FEATURE_EXTRACTORS.register("Factor488RelationMaskFeatureExtractor")
class Factor488RelationMaskFeatureExtractor(ROIBoxMaskFeatureExtractor):
    ...
    def __init__(self, cfg: CfgNode, in_channels: int, out_channels: int):
        ...
        if self.n_dim == 2:
            self.rect_conv = nn.Sequential(
                nn.Conv2d(in_channels, out_channels // 4, kernel_size=7, stride=2, padding=3, bias=True),
                nn.ReLU(inplace=True),
            ...
            self.rect_conv = nn.Sequential(
                nn.Conv3d(in_channels, out_channels // 4, kernel_size=7, stride=2, padding=3, bias=True),
                nn.ReLU(inplace=True),
...
def build_mask_feature_extractor(
        cfg: CfgNode,
        in_channels: int,
        out_channels: int
) -> ROIBoxMaskFeatureExtractor:
    extractor = ROI_BOX_MASK_FEATURE_EXTRACTORS[cfg.MODEL.ROI_BOX_HEAD.MASK_FEATURE_EXTRACTOR]
    return extractor(cfg, in_channels, out_channels)

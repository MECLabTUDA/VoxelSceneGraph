...
This folder contains configuration files and result summaries for reproducing our benchmark experiments across multiple frameworks:
- `nnDetection`: Contains dataset.json and splits_final.pkl files for each dataset, used to train and evaluate with nnDetection.
- `nnUNet`: Same, but for nnUNet.
...
Since the current version of `nnDetection` is mostly out-of-date:
- it relies on older CUDA drivers
- it requires an older version of common Python libraries
- it even uses the V1 of nnUNet
We do not list all the fixes that we had to do to get it working.
For more details about the CT normalization scheme, or disabling the image resampling along the depth, please refer to `nnUNet_Results.md` as both were identical.

Still, we provide the `dataset.json` and `splits_final.pkl` used in this study. We also upload our training.
...
3. Run `scene-graph-prediction/configs/BleedScene3D/make_cfgs.py` to generate the config files for all experiments from the template.
4. Run the experiment for each config file present in `scene-graph-prediction/configs/NeurIPS2025/obj_detec`. For instance:

```bash
sgpred_detector_pretrain_net -c configs/BleedScene3D/obj_detec/all_obj_detec1.yaml
```

### Relation Prediction

5. Run `scene-graph-prediction/configs/BleedScene3D/make_cfgs_rel.py` to generate the config files for all experiments from the template.
6. Before you can do relation prediction, you need to train the corresponding object detector. Check `object_detection_results.md` for more details.
5Run the experiment for each config file present in `scene-graph-prediction/configs/NeurIPS2025/rel_pred`. For instance:

```bash
sgpred_relation_train_net -c configs/BleedScene3D/rel_pred/all_rel_imp_use_gt1.yml
```
